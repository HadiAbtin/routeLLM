...
class DeepSeekProvider(BaseProvider):
    ...
    name = "deepseek"
    supports_attachments = True  # DeepSeek uses OpenAI-compatible API which supports vision
    
    def _build_deepseek_message(self, message: ChatMessage, stored_files: Optional[dict] = None) -> dict:
        """
        Build a DeepSeek message format, handling attachments for multimodal support.
        
        DeepSeek uses OpenAI-compatible API format, so it supports the same multimodal format:
        - {"type": "text", "text": "..."}
        ...
        if message.attachments and stored_files:
            for att in message.attachments:
                ...
                if att.type == "image":
                    # For DeepSeek vision, use the public URL (OpenAI-compatible)
    ...
    async def chat(self, key: str, request: ChatRequest, stored_files: dict = None) -> ChatResponse:
        ...
        Args:
            ...
            stored_files: Optional dict mapping file_id to stored file metadata
        ...
        """
        if not key:
            raise ProviderClientError("DeepSeek API key is not provided.")
        
        settings = get_settings()
        
        # Determine model to use (default from config)
        model = request.model or settings.deepseek_default_model
        
        # DeepSeek API base URL
        base_url = "https://api.deepseek.com/v1"
        
        # Prepare messages for DeepSeek API (with multimodal support if attachments present)
        messages = [
            self._build_deepseek_message(msg, stored_files)
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            try:
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 429:
                        raise ProviderRateLimitError(
                            f"DeepSeek API rate limit: {error_message}",
                            retry_after=retry_after
                        )
                    elif response.status_code >= 500:
                        raise ProviderTransientError(f"DeepSeek API server error: {error_message}")
                        ...
                        raise ProviderAuthenticationError(f"DeepSeek API authentication error: {error_message}")
                    else:
                        raise ProviderClientError(f"DeepSeek API client error: {error_message}")
                ...
                raise ProviderTransientError("Request to DeepSeek API timed out")
            except httpx.RequestError as e:
                logger.error(f"DeepSeek API request error: {str(e)}")
                raise ProviderTransientError(f"Failed to connect to DeepSeek API: {str(e)}")
